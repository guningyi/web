...
function msg_box($message, $title, $destination = 'frm_index.php') {
    ...
    $msg .= '<h2>' . $title . '</h2>';
    $msg .= '<p>' . $message . '</p>';
    $msg .= '<p><a href="' . $destination . '">Yes</a> <a href="frm_index.php">' .
        'No</a></p>';
    $msg .= '</div>';
    return $msg;
}
...
function show_topic($db, $topic_id, $user_id, $limit = 25) {
    ...
    if (mysql_num_rows($result) == 0) {
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            if ($row['update_id'] > 0) {
                $lastupdate = '<p>Last updated: ' . $row['date_updated'] . 
                    ' by ' . $row['updated_by'] . '</p>';
            }
            if ($user_id == $row['author_id'] ||
                ...
                $dellink = '<a href="frm_transact_affirm.php?' . 
                    'action=deletepost&id=' . $row['id'] . '">DELETE</a> ';
            }
            $pcount = '<br/>Posts: ' . ($row['postcount'] == '' ? 0 :
                $row['postcount']);
...
function topic_reply_bar($db, $topic_id, $forum_id) {
    ...
    if ($topic_id > 0) {
        $html .= '<a href="frm_compose.php?forumid=' . $forum_id . '&topicid=' .
            $topic_id . '&reid=' . $topic_id . '">Reply to Thread</a>';
    }
    if ($forum_id > 0) {
