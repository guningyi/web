...
<form action="frm_transact_admin.php" method="post">
 <div>
  <label for="userlist">Please select a user to manage:</label>
...
<p><a href="frm_edit_forum.php">New Forum</a></p>
<?php
    break;

case 'bbcode':
  ?>
<h2>BBcode Administration</h2>
<form method="post" action="frm_transact_admin.php">
 <table>
  <tr>
   <th>Template</th>
    ...
    if (isset($bbcode)) {
        foreach ($bbcode as $key => $value) {
            ...
            echo '<td><input type="button" name="action" id="DelBBCode" ' .
                'value="Delete" onclick="delBBCode(' . $key .
                '); return false;"/></td>';
            echo '</tr>';
        }
    }
