var adsLo,adsDLv='',adsDL='',adsPg='';
try {adsLo=top.location.href}
catch (e){adsLo=(typeof(document.referrer)!='undefined')?document.referrer:''}
adsLo=adsLo||""
adsDLv=adsLo.match(/\?icid=.*?[|](.*?)[|](.*?)[|](.*?)[|]/);
if (adsDLv){
for (var i=1;i<=3;i++)adsDL+=adsDLv[i]+':'
adsDL='kvdl='+adsDL.substring(0,adsDL.length-1)+';';
}
...
document.write('<script type="text/javascript" src="../Movies-list/o.aolcdn.com/ads/'+adsUACH+adsUACN[0]+'"></scr','ipt>')
}else{
if (window.adsIn!=1){
adsIn=1
...
function adSendPg(){
var exDoms=new Array("www.aim.com","www.aol.com","webmail.aol.com","ads.web.aol.com");
if ((adsLo.indexOf('https')==-1)&&(!adInList(adsLo,exDoms))){
adsPg=adsLo.substr(7);
if (adsPg.indexOf('www.')>-1)adsPg=adsPg.substr(4);
adsPg=adsPg.replace(/\.com/,'');
var l=adsPg.indexOf('?')-1;
if (l<=-1)l=adsPg.length;
if (l>45)l=45;
adsPg="kvpg="+escape(adsPg.substr(0,l))+";";
adsPg=adsPg.replace(/\//g,'%2F');
}
}
adSendPg()
...
if (fr.textAd!=1){
var mnum='',x,y,ad_id='|'
if (inH.indexOf('mnum=')>0){
   x=inH.indexOf('mnum=')
   y=inH.indexOf('/',x+1)
   mnum=inH.substring(x+5,y-1)
}
if (inH.indexOf('aolAdId=')>0){
   x=inH.indexOf('aolAdId="')
   y=inH.indexOf(';',x+1)
   ad_id=inH.substring(x+9,y-1)
}
if (fr.mn)aolAdFdBkStr+=fr.mn+'|'+fr.w+'|'+fr.h+'|'+ad_id+'|'+mnum+';';
}
...
fS=ds.indexOf(f)
if (fS!=-1){
if (ds.substring(fS+6,fS+7)>=9){dF='F'}
...
if (this.iframe){ifr.id="adsF"+this.adNum
ifr.src=this.adURL
} else {
...
function adSetupDiv(w,h,adURL,dv1,adPage,dynSz,m){
if (!dv1||dv1==""){
...
if (dynSz&&dynSz!='text'&&dynSz!='iframe'){
...
s+=inc+adsATOth+adsSrAT+adsATWM+adsTable+adsDL+adsPg+"kvmn="+m+";target=_blank;"+adsTzAT+"grp="+adsScr+";misc="+adsScr
...
adSetupDiv(w,h,s,divName,fName,'text',m)
adsDivs[adsDivs.length-1].LoadAd()
}
else if (t=='ajax'){
adsAJAX=1
adSetupDiv(w,h,s,divName,fName,dynSz,m)
adsDivs[adsDivs.length-1].LoadAd()
}
else if (t=='iframe'){
var s1=s.replace(/addyn\/3.0/,"adiframe/3.0")
adSetupDiv(w,h,s1,divName,fName,'iframe',m)
adsDivs[adsDivs.length-1].LoadAd()
}
else{if (adsTp=='F'){
adSetupDiv(w,h,s,divName,fName,dynSz,m)
...
adSetupDiv(w,h,aHref,divName,imgSrc,dynSz,m)
adsDivs[adsDivs.length-1].LoadAd()
}}
adsTile++
adsNMSG=adsNMSG_O
