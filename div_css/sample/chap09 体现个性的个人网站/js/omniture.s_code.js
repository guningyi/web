...
var s_account="corbisveerdev";
var host = window.location.host;
if(host && host.indexOf('www.veer.com') != -1){
	s_account="corbisveer";
}
...
s.linkInternalFilters="javascript:,veer.com"
...
function s_doPlugins(s) {
	...
	s.campaign=s.getQueryParam('cid'); //remove comments and replace 'cid' with campaign query parameter
	s.campaign=s.getValOnce(s.campaign,'s_campaign',30);
	
	/* Plugin: Time Parting - PST */
	s.prop2=s.getTimeParting('h','-8',new Date().getFullYear()); // Set hour 
	s.prop1=s.getTimeParting('d','-8',new Date().getFullYear()); // Set day
	s.prop3=s.getTimeParting('w','-8',new Date().getFullYear()); // Set Weekend / Weekday
	
	/* Plugin: getNewRepeat 1.0 */
	s.prop7=s.getNewRepeat();
	
	/* Set page name in eVar1 */
	s.eVar1=s.pageName;
	
	/*Plugin: getTimeToComplete 0.4*/ //Add to Cart > Purchase
	if(s.events.indexOf('scAdd')>-1)
		s.prop10='start';
	if(s.events.indexOf('purchase')>-1)
		s.prop10='stop';
	s.prop10=s.getTimeToComplete(s.prop10,'ttc',30);
	
	/* Copy s.props into eVars */
	if (s.prop4) 	{s.eVar3=s.prop4;};
	if (s.prop18)	{s.eVar4=s.prop18;};
	if (s.prop23)	{s.eVar6=s.prop23;};
	if (s.prop5)	{s.eVar9=s.prop5;};
	if (s.prop8)	{s.eVar10=s.prop8;};
	if (s.prop9)	{s.eVar11=s.prop9;};
...
s.visitorNamespace="corbis"
s.trackingServer="metrics.veer.com"
s.trackingServerSecure="smetrics.veer.com"
s.dc="122"
