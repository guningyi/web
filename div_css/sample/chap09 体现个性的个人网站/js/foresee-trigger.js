var ForeSee = {
    ...
    'enabled': true,
    'files': '/foresee/',
    //'files': 'http://www.veer.com/foresee/',
    'id': 'IF91dE44JZl1w8wJh4YBkg==',
    'sites': [{
        path: 'veer.com',
        cookie: 'session',
        domain: 'veer.com'
...
this.save();return this},fsr$empty:function(){this.hash.fsr$empty();this.save();return this}});ForeSee.Asset=new ForeSee.Hash({src:function(A){var B=A;
if(A.substring(0,2)=="//"){B=document.location.protocol+B}return B},javascript:function(F,D){D=ForeSee.$extend({onload:ForeSee.$empty,document:document,check:ForeSee.$lambda(true)},D);
var B=new ForeSee.Element("script",{src:ForeSee.Asset.src(F),type:"text/javascript"});var E=D.onload.fsr$bind(B),A=D.check,G=D.document;
...
}D=D.onload=D.onabort=D.onerror=null;G.fsr$delay(1,A,A);A.fsr$fireEvent(E,A,1)}});D.src=ForeSee.Asset.src(C);
if(A.src!=D.src){A.src=D.src}if(D&&D.complete){D.onload.fsr$delay(1)}return A.fsr$setProperties(B)
},css:function(B,A){return new ForeSee.Element("link",ForeSee.$merge({rel:"stylesheet",media:"screen",type:"text/css",href:ForeSee.Asset.src(B)},A)).fsr$inject(document.getElementsByTagName("head")[0])
...
},fsr$toQueryString:function(){var G=ForeSee.c();var D=G.fsr$get("browser");var F={browser:D.name+" "+D.version,os:D.platform,pv:G.fsr$get("pv"),url:G.fsr$get("current"),ref_url:G.fsr$get("ref_url"),locale:G.fsr$get("locale")||"",site:G.fsr$get("site")||"",referrer:G.fsr$get("referrer")||"",terms:G.fsr$get("terms")||"",sessionid:G.fsr$get("fsrid")||""};
...
}this.running=false;this.fsr$fireEvent("cancel");return this}});ForeSee.RR=new ForeSee.Hash({hasRR:function(){if(typeof robotreplay!="undefined"){return true
}return false},setOKTransmit:function(){if(this.hasRR()){var A=robotreplay.Session.rr_group_session_id;
robotreplay.Log.setOKTransmit();ForeSee.CPPS.fsr$set("replay_id",A)}},recordEvent:function(A,B){if(this.hasRR()){robotreplay.Log.push(A,B)
}},cancelRecord:function(){if(this.hasRR()){robotreplay.Log.cancelRecord()}}});ForeSee.services={survey:{host:"www.foreseeresults.com",path:"/survey",url:"/display"},check:{host:"controller.foreseeresults.com",path:"/fsrSurvey",url:"/OTCImg",success:3},event:{host:"events.foreseeresults.com",path:"/rec",url:"/process",enabled:false,id:false},domain:{host:"www.foreseeresults.com",path:"/survey",url:"/FSRImg",success:3}};
...
};ForeSee.log=function(B,A){if(!ForeSee.services.event.enabled){return }var C=ForeSee.c();new ForeSee.RemoteEvent("logit",{host:ForeSee.services.event.host,path:ForeSee.services.event.path,url:ForeSee.services.event.url}).send({cid:ForeSee.id,fsrid:C.fsr$get("fsrid"),name:C.fsr$get("sd").name,msg:B,param:A,tmz:new Date().getTimezoneOffset()})
...
}});break}}});return G},init:function(){fsr$dbug.log("======INIT=======");var G=ForeSee.c();var J=G.fsr$get("alive")||0;
J=J+1;G.fsr$set("alive",J);this.ralive=true;G=ForeSee.c();J=G.fsr$get("alive");if(!J){fsr$dbug.log("Exit...cookies are not enabled.");
return false}fsr$dbug.log("alive: "+J);if(!this.trackerRunning()){clearInterval(fsr$timer);ForeSee.Cookie.dispose("foresee.alive",{path:"/",domain:ForeSee.site.domain||false})
}var E=ForeSee.Browser;G.fsr$set("browser",{name:ForeSee.Browser.Type.name,version:ForeSee.Browser.Type.version,platform:ForeSee.Browser.Platform.os});
fsr$dbug.log("browser: "+E.Type.name+" "+E.Type.version+" on "+E.Platform.os);if(ForeSee.UnsupportedBrowsers[E.Type.name]){if(E.Type.version<=ForeSee.UnsupportedBrowsers[E.Type.name]){fsr$dbug.log("Browser not surpported.");
return false}}if(this.validateIP()==0){fsr$dbug.log("Invalid IP Address.");return false}var I;if(ForeSee.$defined(G.fsr$get("finish"))){var F=G.fsr$get("timeout");
var A=((ForeSee.$time()-G.fsr$get("finish"))/1000);fsr$dbug.log("ptimeout: "+F);fsr$dbug.log("loadtime: "+A);
I=(0.9*F)+(0.1*(A*2));if(I<2){I=2}else{if(I>5){I=5}}}else{I=ForeSee.$P().tracker.timeout}G.fsr$set("timeout",I);
fsr$dbug.log("timeout: "+I);if(!G.fsr$get("start")){var L;if(L=ForeSee.Cookie.read("foresee.repeatdays",{path:"/",domain:ForeSee.site.domain||false})){fsr$dbug.log("Persistent Cookie Found: "+L);
return false}G.fsr$set("start",ForeSee.$time());var K=this;this.dhtml_win=1;new ForeSee.Asset.javascript(ForeSee.files+"foresee-dhtml-popup.js",{id:"foresee-dhtml-popup",onload:function(){fsr$dbug.log("DHTML popup script loaded (1).");
K.dhtml_win=2}});this.dhtml_css=1;new ForeSee.Asset.css(ForeSee.files+ForeSee.$P().invite.css);if(ForeSee.$P().invite.css!=ForeSee.$P().qualifier.css){new ForeSee.Asset.css(ForeSee.files+ForeSee.$P().qualifier.css)
}this.dhtml_css=2;var B=this.generateid();G.fsr$set("fsrid",B);if(document.referrer&&document.referrer!=""){G.fsr$set("ref_url",document.referrer);
var D=document.referrer.match(/^(\w+\:\/\/)?(((\w+\.?))+)\//);var H="";if(D&&D.length>=3){H=D[2]}G.fsr$set("referrer",H);
fsr$dbug.log("referrer: "+H);var C=this.decodeReferrer(document.referrer);G.fsr$set("terms",C);fsr$dbug.log("search terms: "+C)
}}fsr$dbug.log("invite: "+(ForeSee.$pick(G.fsr$get("invite"),"")));fsr$dbug.log("tracker: "+(ForeSee.$pick(G.fsr$get("tracker")||"")));
...
var value=p[j].value;if(ForeSee.$type(name)!="array"){name=[name];value=[value]}var v,z=true;for(var x=0,y=name.length;
x<y;x++){if(v=eval(name[x])){if(!v.match(value[x])){z=false;break}}}if(z){m[k++]=i;break}}if(k!=pk){break
}if(pin[d[i].name]){m[k++]=i}if(k!=pk){break}}return m},invite:function(){ForeSee.invite=0;if(ForeSee.locale){ForeSee.c().fsr$set("locale",ForeSee.locale)
}var A=this;if(this.sd.invite){(function(){ForeSee.log(100,ForeSee.c().fsr$get("current"));A.prepareDHTML("invite","accepted","declined","closed")
}).fsr$delay((this.sd.invite.delay||0)*1000)}else{(function(){A.accepted();A.closed()}).fsr$delay(0)
}},prepareDHTML:function(E,F,C,B){var A=this;if(!ForeSee.$defined(this.dhtml_css)){this.dhtml_css=1;
new ForeSee.Asset.css(ForeSee.files+this.sd[E].css);this.dhtml_css=2}if(!ForeSee.$defined(this.dhtml_win)){this.dhtml_win=1;
...
}ForeSee.RR.setOKTransmit();var A=this;A.processAccept()},declined:function(){ForeSee.invite=-1;ForeSee.log(102);
ForeSee.c().fsr$set("invite",ForeSee.invite);ForeSee.RR.cancelRecord()},closed:function(){ForeSee.c().fsr$set("invite",ForeSee.invite);
if(this.sd.repeatdays){ForeSee.Cookie.write("foresee.repeatdays",this.sd.repeatdays,{path:"/",domain:ForeSee.site.domain||false,duration:this.sd.repeatdays})
...
}},generateid:function(){var A="";if(!ForeSee.services.event.enabled){return A}if(!ForeSee.services.event.id){return A
}A=((new Date())-0)+"_"+Math.round(Math.random()*100000000);return A},decodeReferrer:function(A){A=decodeURIComponent(A);
var C=null;var B=document.referrer.match(/[?&]q=([^&]*)/)||document.referrer.match(/[?&]p=([^&]*)/)||document.referrer.match(/[?&]query=([^&]*)/);
