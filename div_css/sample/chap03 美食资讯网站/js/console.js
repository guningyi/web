    ...
    Console: Class.create({
        initialize: function () {
            if (!LFM.get("config", 'DEVELOPMENT_SERVER')) {
                // On live this is just a wrapper.
                return;
            }
            var cssposition = Prototype.Browser.IE ? "absolute" : "fixed";
            
            this.container = new Element("div", {id: "lfmConsole"});
            this.messageList = new Element("ul");
            this.container.insert(this.messageList);
            this.wrapper = this.messageList.wrap("div").addClassName("wrapper");
            this.resizeHandle = new Element("div", {id: "lfmConsoleResizeHandle"});
            this.container.insert(this.resizeHandle);
            this.userDefinedMaxHeight = false;
            
            var foo = new Draggable(this.container, {
                starteffect: false,
                endeffect: false,
                handle: this.messageList,
                onEnd: function(draggable) {
                    if (Prototype.Browser.IE) {
                        var position = draggable.element.cumulativeOffset();
                        position.top = position.top - (ignoreMe2 = document.documentElement.scrollTop ? document.documentElement.scrollTop : document.body.scrollTop);
                        position.right = document.viewport.getWidth() - position.left - draggable.element.getWidth();
                        draggable.element.style.setExpression("top", "(" + position.top + " + (ignoreMe = document.documentElement.scrollTop ? document.documentElement.scrollTop : document.body.scrollTop)) + 'px'");
                        draggable.element.style.setExpression("right", "(" + position.right + " + (ignoreMe2 = document.documentElement.scrollLeft ? document.documentElement.scrollLeft : document.body.scrollLeft)) + 'px'");
                        LFM.setCookie(
                            "lfmConsole",
            ...
            this.container.setStyle({position: cssposition});
            var bar = new Resizable(this.container, {
                handle: this.resizeHandle,
                starteffect: false,
                endeffect: function(element) {
                    this.userDefinedMaxHeight = this.container.getHeight() - 23;
                    this.container.setStyle({position: cssposition});
                    this.wrapper.setStyle({
                        height: this.userDefinedMaxHeight + "px"
                    });
                }.bind(this)
            });
            
            this.tip = new Element("div", {id: "lfmConsoleTip"});
            this.setTip("ctrl+d toggles the console");
            this.container.insert(this.tip);
            
            document.observe("keydown", function(event) {
                if (event.ctrlKey) {
                    var LETTER_D = 68;
                    if (Prototype.Browser.IE) {
                        if (event.keyCode == LETTER_D) {
                            this.toggle();
                        }
                    } else {
                        if (event.which == LETTER_D) {
                            this.toggle();
                        }
                    }
                }
            ...
            document.observe("dom:loaded", function (e) {
                ...
                $$("body")[0].insert({top: this.container});
            }.bind(this));
        },
        customise: function () {
            var cookie = LFM.getCookie("lfmConsole");
            if (cookie) {
                cookie = cookie.split(",");
                ...
                if (position.top > viewportDimensions.height) {
                    position.top = viewportDimensions.height - 25;
                }
                if (position.right > viewportDimensions.width) {
                    position.right = viewportDimensions.width - 25;
                }
                if (Prototype.Browser.IE) {
                    this.container.style.setExpression("top", "(" + position.top + " + (ignoreMe = document.documentElement.scrollTop ? document.documentElement.scrollTop : document.body.scrollTop)) + 'px'");
                    this.container.style.setExpression("right", "(" + position.right + " + (ignoreMe2 = document.documentElement.scrollLeft ? document.documentElement.scrollLeft : document.body.scrollLeft)) + 'px'");
                } else {
        ...
        getStack: function () {
            try {
                ...
                return (e.stack || '').match(this.Regexp).reject(function(path) {
                    return (/(prototype|scriptaculous|console)\.js/).test(path);
                }).join("\n");
            }
        },
        setHeight: function (value) {
            if (!this.userDefinedMaxHeight) {
                var height = this.messageList.getHeight() > 300 ? "300px" : "auto";
        ...
        log: function () {
            ...
            if (Object.isElement(text)) text = text.tagName;
            message.update(text + "");
            if (!this.messageList.childElements().length) message.className = "first";
            this.messageList.insert(message);
            this.setHeight();
            this.wrapper.scrollToDescendant(this.messageList.lastChild, {duration: 0});
            // IE needs to redraw this…
            this.tip.toggle().toggle();
        }
    })
});
LFM.Utility.Console.prototype.warn = LFM.Utility.Console.prototype.log;
LFM.Utility.Console.prototype.info = LFM.Utility.Console.prototype.log;
LFM.Utility.Console.prototype.error = LFM.Utility.Console.prototype.log;
LFM.Utility.Console.prototype.dir = LFM.Utility.Console.prototype.log;
...
//     console = new LFM.Utility.Console();
// }